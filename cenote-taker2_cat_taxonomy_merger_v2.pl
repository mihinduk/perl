#!/usr/bin/env perl
# cenote-taker2_cat_taxonomy_merger_v2.pl
# 2020_07_16
use warnings;
use strict;
...
my $base = $ARGV[3];
my $comp = "$base". "_CT2_CAT_comp_v2.txt";
my $final_taxonomy = "$base". "_CT2_CAT_contig_taxonomy_v2.txt";
...
while (my $line = <IN>) {
  ...
  if (defined($CT_taxid{$ocn})) {
    ...
    } elsif (($ct_Kingdom eq "Viruses") && ($Kingdom eq "Bacteria"))  { # If CT2 eq Viruses CAT eq bacteria, choose CT2
        $final_tax = $CT_out{$ocn};
        print OUT "$final_tax\t$Coverage\t$Repetitive\n"; $ct_count++;  $rule_9++;
        print OUT_2 "$final_tax\tFLAG\n$line\n";
    } elsif (($ct_Kingdom eq "Viruses") && ($Kingdom ne "Viruses" || "Bacteria")) {
